...
/datum/component/footstep/proc/play_footstep()
	...
	if(isshoefoot(LM))
		...
		if(ishuman(LM)) //for proper humans, they're special
			...
			if(((!H.shoes && !feetCover) || !(H.mobility_flags & MOBILITY_STAND) && !(LM.throwing || LM.movement_type & (VENTCRAWLING | FLYING) || (LM.stat != CONSCIOUS) || LM.incapacitated() || LM.IsStun() || LM.IsParalyzed()))) //are we NOT wearing shoes or are we lying/crawling (using hands to move around) and not immobilized?
				playsound(T, pick(GLOB.barefootstep[T.barefootstep][1]),
					GLOB.barefootstep[T.barefootstep][2] * v,
					TRUE,
					GLOB.barefootstep[T.barefootstep][3] + e)